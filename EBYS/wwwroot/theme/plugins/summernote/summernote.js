...
var dialog = renderer["a" /* default */].create('<div class="moBusiness note-moBusiness" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
  ...
  $node.html(['<div class="moBusiness-dialog">', '<div class="moBusiness-content">', options.title ? '<div class="moBusiness-header">' + '<button type="button" class="close" data-dismiss="moBusiness" aria-label="Close" aria-hidden="true">&times;</button>' + '<h4 class="moBusiness-title">' + options.title + '</h4>' + '</div>' : '', '<div class="moBusiness-body">' + options.body + '</div>', options.footer ? '<div class="moBusiness-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
});
